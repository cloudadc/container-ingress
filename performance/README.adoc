...
== F5 CIS Service Discover Performance
...
=== TMSH commands used to count time

[source, bash]
.*Count totoal pool*
----
for i in {1..1000} ; do date ; tmsh list ltm pool /k8s_AS3/Shared/* | grep pool | wc -l ; sleep 3 ;  done
----

[source, bash]
.*Count specific pool's total pool member*
----
for i in {1..100} ; do date ; tmsh list ltm pool /k8s_AS3/Shared/ingress_perftest139_app_svc | grep address | wc -l; sleep 3 ;  done
----
...
.*Results*
----
total_namespaces,total_deploy_time,service_discover_time
20,5,3
40,5,5
60,6,6
80,10,9
