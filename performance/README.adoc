...
[source, bash]
.*Tmsh command: count total pool*
----
STARTTIME=$(date +%s) ; for i in {1..100} ; do tmsh list ltm pool /k8s_AS3/Shared/* | grep pool | wc -l ; ENDTIME=$(date +%s); echo "spend $(($ENDTIME - $STARTTIME)) seconds" ; sleep 3 ; done
----

[source, bash]
.*Tmsh command: count specific pool's total pool member*
----
STARTTIME=$(date +%s) ; for i in {1..100} ; do tmsh list ltm pool /k8s_AS3/Shared/ingress_perftest299_app_svc | grep address | wc -l;  ENDTIME=$(date +%s); echo "spend $(($ENDTIME - $STARTTIME)) seconds" ; sleep 3 ;  done
----
...
.*Results*
----
total_namespaces,total_deploy_time,service_discover_time,total_deleletion_time
20,12,3,4
40,18,5,5
60,21,6,6
80,25,9,7
...
image:img/ingress-perf-multi-ns.png[]
...
kubectl apply -f 120-ns-configmap.yaml
kubectl apply -f 140-ns-configmap.yaml
kubectl apply -f 160-ns-configmap.yaml
kubectl apply -f 180-ns-configmap.yaml
kubectl apply -f 200-ns-configmap.yaml
----
